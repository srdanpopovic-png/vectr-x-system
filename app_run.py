...
# --- URL PARAMETER CHECK (ATHLETEN-MODUS) ---
...
def get_benchmark_html(val, metric_type, color_hex):
    # --- VECTR-X "HARD TRUTH" BENCHMARKS (2026 STANDARDS) ---
    
    # 1. VO2MAX: Strenger. 65 ist die neue 60.
    if metric_type == "vo2max": 
        levels = [("ELITE", 65, 999), ("ATHLETE", 55, 65), ("AMATEUR", 45, 55), ("ROOKIE", 0, 45)]
    
    # 2. SCHWELLE (LT2): Wer "Elite" sein will, muss 3:25/km laufen können.
    elif metric_type == "lt2": 
        levels = [("ELITE", 17.5, 99), ("ATHLETE", 15.0, 17.5), ("AMATEUR", 12.0, 15.0), ("ROOKIE", 0, 12.0)]
    
    # 3. FATMAX: 4:00/km Pace an der aeroben Schwelle ist der Benchmark.
    elif metric_type == "fatmax": 
        levels = [("ELITE", 15.0, 99), ("ATHLETE", 12.5, 15.0), ("AMATEUR", 10.0, 12.5), ("ROOKIE", 0, 10.0)]
    
    # 4. SPEED-TAX (NEU): mmol/l pro km/h
    # < 0.30: Ultra-Effizient (Diesel)
    # 0.30 - 0.55: Solider Hybrid-Motor
    # > 0.85: Teurer Spritfresser
    elif metric_type == "re": 
        levels = [("ELITE", 0, 0.30), ("ATHLETE", 0.30, 0.55), ("AMATEUR", 0.55, 0.85), ("ROOKIE", 0.85, 9.99)]
    
    # 5. RESILIENZ: Angepasst an hyperbolische Formel
    # > 75% ist nur mit extrem flachem High-End-Gradienten (Slope < 0.22) möglich.
    ...
    for label, low, high in levels:
        ...
        # Text-Formatierung
        if metric_type == "re":
            # Bei Speed-Tax ist niedriger besser -> "<" Logik umdrehen
             range_txt = f"<{high}" if low == 0 else f">{low}" if high > 5 else f"{low}-{high}"
        else:
...
st.markdown("""
    ...
    /* CYBER SHARE BUTTON (PURPLE/MAGENTA) */
    .share-btn { 
...
# --- DATEN-EXTRAKTION (URL ODER STANDARD) ---
...
if not is_athlete:
    with st.sidebar:
        ...
        weight = st.number_input(t("Gewicht (kg)", "Weight"), value=w_def, step=0.5)
        height = st.number_input(t("Größe (cm)", "Height"), value=h_def, step=1.0)
        sw = st.number_input(t("Schulterbreite (cm)", "Shoulder"), value=s_def, step=0.5)
        ...
        # --- DER CYBER BUTTON ---
        st.write("---")
        share_query = urllib.parse.urlencode({'w': weight, 'h': height, 's': sw, 'v': ",".join(map(str,v1)), 'l': ",".join(map(str,l1)), 'hr': ",".join(map(str,h1))})
...
if metrics_t1:
    ...
    with tabs[0]: # ANALYSE
        st.markdown(f"### // {t('BIOMETRISCHER_SCAN', 'BIOMETRIC_SCAN')}")
        
        cols = st.columns(4)
        m_show = [
            (t("BASE // FATMAX", "BASE // FATMAX"), "fatmax", "KM/H", True),
            (t("SCHWELLE // iANS", "THRESHOLD // iANS"), "lt2", "KM/H", True),
            (t("SPEED-TAX", "SPEED-TAX"), "re", "MMOL/KMH", False),
        ...
        for col, (lab, k, unit, show_hf) in zip(cols, m_show):
            ...
            with col:
                ...
                <div class="metric-wrapper">
                    <span style='color:#8E8E93; font-size:13px; font-weight:600;'>{lab}</span>
                    <div class='{glow_class}' style='font-size:32px; font-weight:700;'>{v_disp} <span style='font-size:14px;'>{unit}</span></div>
                    {pace_html}{hf_html}{delta_html}
        ...
        if metrics_t2: 
            ax.plot(metrics_t2["v_fine"], metrics_t2["l_fine"], '--', color='#6E3CBC', lw=1.5, alpha=0.5, label=t('ARCHIV', 'ARCHIVE'))
        ...
        ax2 = ax.twinx()
        ax2.plot(metrics_t1["v_fine"], metrics_t1["h_fine"], ':', color='#FF3131', alpha=0.6, lw=1.5)
        
        ax.grid(True, color='#1C1C1E', lw=0.5)
        st.pyplot(fig)
        plt.close(fig)
        
        # --- METABOLISCHE RESILIENZ (STATUS BOX) ---
        s_val = metrics_t1['slope']
        if s_val < 0.45: 
            res_status, res_class = t("ULTRA STABIL", "ULTRA STABLE"), "res-ultra"
        elif s_val < 0.75: 
            res_status, res_class = t("METABOLISCH RESILIENT", "METABOLIC RESILIENT"), "res-stable"
        elif s_val < 1.1: 
            res_status, res_class = t("STABILITÄTS-LIMIT", "STABILITY LIMIT"), "res-limit"
        else: 
            res_status, res_class = t("KRITISCHER BEREICH", "CRITICAL ZONE"), "res-critical"
            
        st.markdown(f"""
        <div class="stability-box {res_class}">
            // {t('METABOLISCHE RESILIENZ', 'METABOLIC RESILIENCE')} // <br>
            <span style="font-size:18px; font-weight:700;">{res_status}</span><br>
            <span style="font-size:12px; opacity:0.8;">VECTR-SCORE: {int(metrics_t1['stab'])}%</span>
        </div>""", unsafe_allow_html=True)
    ...
    with tabs[1]: # ZONEN
        ...
        z_data = [
            ("blue-neon", t("RECOVERY", "RECOVERY"), f"< {fmax*0.9:.1f} KM/H", f"> {fmt_pace(fmax*0.9)}", f"< {hf_f-10}"), 
            ("green-neon", t("LONG RUN", "LONG RUN"), f"{fmax*0.9:.1f}-{l1:.1f} KM/H", f"{fmt_pace(fmax*0.9)}-{fmt_pace(l1)}", f"{hf_f-10} - {hf1}"), 
            ("yellow-neon", t("TEMPO", "TEMPO"), f"{l1:.1f}-{l2*0.95:.1f} KM/H", f"{fmt_pace(l1)}-{fmt_pace(l2*0.95)}", f"{hf1} - {int(hf2*0.95)}"), 
            ("orange-neon", t("SCHWELLE", "THRESHOLD"), f"{l2*0.95:.1f}-{l2*1.05:.1f} KM/H", f"{fmt_pace(l2*0.95)}-{fmt_pace(l2*1.05)}", f"{int(hf2*0.95)} - {int(hf2*1.03)}"), 
            ("red-neon", t("HIT", "HIT"), f"> {l2*1.05:.1f} KM/H", f"< {fmt_pace(l2*1.05)}", f"> {int(hf2*1.03)}")
        ]
    ...
    with tabs[2]: # PROGNOSE
        st.markdown(f"### // {t('PROGNOSE', 'FORECAST')}")
    ...
    with tabs[3]: # SET CARD
        ...
        # 2. ZEILE (MITTE): iANS (Links) | SPEED-TAX (Rechts)
        c1, c2 = st.columns(2)
        with c1: 
            # FATMAX (Hierher getauscht)
        ...
        # 3. ZEILE (UNTEN): FATMAX (Links) | RESILIENZ (Rechts)
        c3, c4 = st.columns(2)
        with c3: 
            # iANS (Hier, weil du Fatmax und iANS tauschen wolltest)
